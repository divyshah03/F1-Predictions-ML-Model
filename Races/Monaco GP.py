"""
F1 Race Prediction Model - Monaco GP Analysis
Predicts race performance using qualifying times, sector times, weather data, and position change history.
"""
...
def get_average_position_change_monaco() -> dict:
    """
    Get average position change data for Monaco GP.
    
    Returns:
        Dictionary mapping driver codes to average position change
    """
    return {
...
def create_qualifying_dataframe() -> pd.DataFrame:
    """
    Create DataFrame with 2025 Monaco GP qualifying times.
...
def prepare_training_data(
    ...
    position_change = get_average_position_change_monaco()
    ...
    qualifying_df["AveragePositionChange"] = qualifying_df["Driver"].map(position_change)
...
def display_results(predictions_df: pd.DataFrame, mae: float):
    ...
    podium = predictions_df.loc[:2, ["Driver", "PredictedRaceTime (s)"]]
